...
@section('title','Buat Password Baru - Belidigi')
@section('page_title','Buat Password Baru')
        ...
        <div class="flex flex-wrap -mx-4">
            <div class="w-full px-4">
                <div class="max-w-[525px] mx-auto text-center bg-white rounded-lg relative overflow-hidden py-14 px-8 sm:px-12 md:px-[60px] wow fadeInUp" data-wow-delay=".15s">
                    ...
                    <form action="{{ route('update_new_password') }}" method="POST" id="new_user_form">
                        @csrf
                        <input type="text" value="{{ Session::get('key'); }}" hidden name="hash" id="hash">
                        <input type="text" value="{{ Session::get('username'); }}" hidden name="username" id="username">
                        <div class="mb-6">
                        ...
                        <div class="mb-10">
                            <button id="addUser_btn" class="w-full rounded-md border bordder-primary py-3 px-5 bg-primary text-base text-white cursor-pointer hover:shadow-md transition duration-300 ease-in-out" />Perbaharui Password</button>
                        </div>
                    </form>
                    <script>
                        ...
                        $('#addUser_btn').click(function(event){
                            ...
                            if ($('#password').val().length < 8 || !$('#password').val().match(upperCase) || !$('#password').val().match(lowerCase) || !$('#password').val().match(numbers) || !$('#password').val().match(specialChar) || $('#repassword').val() != $('#password').val() ){
                                Swal.fire({
                                    title: "Oops!",
