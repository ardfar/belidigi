...
@section('page_title','Halaman Pencarian')
      ...
      <div class="flex flex-wrap -mx-4">
        <div class="w-full px-4">
          <div class="text-center mx-auto mb-[60px] lg:mb-20 max-w-[620px]">
            ...
            <form action="{{ route('search_transaction_data') }}">
                ...
                <div class="relative inline-block w-full text-gray-700 px-2 py-5">
                    ...
                    <a href="{{ route('cari-transaksi') }}" class="text-base text-white bg-black rounded-lg px-6" style="padding: .85rem 1.5rem; line-height:1.5rem;background:#dc3741"> Reset </a>
                </div>
            </form>
      ...
      <div class="flex flex-wrap -mx-4">
        <div class="w-full lg:w-full px-4">
          <div class="cursor-pointer single-faq w-full bg-white border border-[#F3F4FE] rounded-lg p-2 sm:p-8 mb-2">
            <div class="faq-btn flex items-center w-full text-left">
              <div class="w-full" id="gridTable">
                  ...
                  <div style="text-align: center;font-weight:bolder">EMAIL</div>
                  <div id="specialGridTable" style="text-align: center;font-weight:bolder">NO HP</div>
          ...
          @foreach ($generalPurchaseInfo as $generalInfo)
            <div class="cursor-pointer single-faq w-full bg-white border border-[#F3F4FE] rounded-lg p-2 sm:p-8 mb-8 wow fadeInUp" data-wow-delay=".1s " onclick="authToView('{{ $generalInfo->idTransaksi }}')">
                <div class="faq-btn flex items-center w-full text-left">
                  <div class="w-full" id="gridTable">
                      <div style="text-align: center">{{ $generalInfo->created_at }}</div>
                      <div style="text-align: center">{{ Str::mask(Str::limit($generalInfo->email,15),'*',5) }}</div>
                      <div id="specialGridTable" style="text-align: center">@if ($generalInfo->hp == '') <b>-</b> @else {{ Str::mask($generalInfo->hp,'*',7,5) }} @endif</div>
